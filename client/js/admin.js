...
async function loadParking() {
  ...
  try {
    ...
    const pendingCount = _allParking.filter(p => !p.isApproved).length;
...
function renderBookingsTable(bookings) {
  ...
  tbody.innerHTML = bookings.map(b => {
    const canCancel = b.status === 'active' || b.status === 'confirmed';
    const actionBtn = canCancel
      ? `<button class="btn btn-ghost btn-sm" style="color:var(--amber)" onclick="cancelBooking(this,${b.id},'${escHtml(b.bookingCode)}')">Cancel</button>`
      : '';
    const hrs = b.durationHours ? parseFloat(b.durationHours).toFixed(1) + ' hrs' : '—';
    return `
      <tr data-status="${escHtml(b.status)}" data-id="${b.id}">
        ...
        <td>${hrs}</td>
        <td>${fmtCurrency(b.totalAmount)}</td>
        <td><span class="badge ${b.paymentStatus==='paid'?'badge-green':'badge-red'}">${b.paymentStatus||'—'}</span></td>
        <td>${statusBadge(b.status)}</td>
...
async function cancelBooking(btn, id, code) {
  if (!confirm(`Cancel booking ${code}?`)) return;
  btn.disabled = true;
  try {
    await apiCall('PATCH', `/users/admin/bookings/${id}/cancel`);
    toast(`Booking ${code} cancelled`, 'info');
    loadBookings();
  } catch(e) {
    toast('Error: ' + e.message, 'error');
    btn.disabled = false;
