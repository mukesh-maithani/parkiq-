...
exports.verifyPayment = async (req, res, next) => {
    try {
        ...
        // Step 4b: Credit owner's total_earnings with 85% of total_amount; admin keeps 15%
        const bookingRow = await query('SELECT total_amount, parking_id FROM bookings WHERE id = ?', [payment.booking_id]);
        if (bookingRow.length > 0) {
            const { total_amount, parking_id } = bookingRow[0];
            const ownerEarning = Math.round(parseFloat(total_amount) * 0.85 * 100) / 100;
            await query(`
                UPDATE parking_owners po
                JOIN parking_locations pl ON pl.owner_id = po.user_id
                SET po.total_earnings = po.total_earnings + ?
                WHERE pl.id = ?
            `, [ownerEarning, parking_id]);
        }
...
exports.razorpayCallback = async (req, res) => {
    ...
    try {
        ...
        // Credit owner's total_earnings with 85% of total_amount; admin keeps 15%
        const bookingRow2 = await query('SELECT total_amount, parking_id FROM bookings WHERE id = ?', [payment.booking_id]);
        if (bookingRow2.length > 0) {
            const { total_amount, parking_id } = bookingRow2[0];
            const ownerEarning2 = Math.round(parseFloat(total_amount) * 0.85 * 100) / 100;
            await query(`
                UPDATE parking_owners po
                JOIN parking_locations pl ON pl.owner_id = po.user_id
                SET po.total_earnings = po.total_earnings + ?
                WHERE pl.id = ?
            `, [ownerEarning2, parking_id]);
        }
